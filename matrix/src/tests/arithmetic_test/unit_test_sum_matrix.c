...
START_TEST(sum_matrix_8) {
  ...
  int columns = 2;
  double arrayA[] = {-10.0, -20.0, -50.0, -10.0};
  double arrayB[] = {1.0, sqrt(-1), 5.0, 0.0};
...
Suite *s21_sum_matrix_suite(void) {
  ...
  Suite *test_suite = suite_create("matrix addition");

  /*Создаём тест-кейсы, в которые сложим все тестовые случаи по темам и добавим
   * их в тестовый набор*/
  TCase *case_create = tcase_create("Matrix addition");
  tcase_add_test(case_create, sum_matrix_1);
  tcase_add_test(case_create, sum_matrix_2);
  tcase_add_test(case_create, sum_matrix_3);
  tcase_add_test(case_create, sum_matrix_4);
  tcase_add_test(case_create, sum_matrix_5);
  tcase_add_test(case_create, sum_matrix_6);
  tcase_add_test(case_create, sum_matrix_7);
  tcase_add_test(case_create, sum_matrix_8);
  tcase_add_test(case_create, sum_matrix_9);
