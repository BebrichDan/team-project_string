...
START_TEST(test_s21_sprintf_E_number_with_maximal_precision) {
  char buffer_standard[100];
  char buffer_custom[100];
  double expected = 0.00123456789;

  sprintf(buffer_standard, "Number with maximal precision: %.15E", expected);
  s21_sprintf(buffer_custom, "Number with maximal precision: %.15E", expected);
