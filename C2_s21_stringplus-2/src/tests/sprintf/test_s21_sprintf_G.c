...
START_TEST(test_s21_sprintf_G_basic_double) {
  ...
  sprintf(buffer_standard, "Double: %.15G", expected);
  s21_sprintf(buffer_custom, "Double: %.15G", expected);
...
START_TEST(test_s21_sprintf_G_space_flag) {
  ...
  sprintf(buffer_standard, "Double with space flag: % G", expected);
  s21_sprintf(buffer_custom, "Double with space flag: % G", expected);
...
START_TEST(test_s21_sprintf_G_width_field) {
  char buffer_standard[100];
  char buffer_custom[100];
  long double expected = 434543532.334543;

  sprintf(buffer_standard, "Double with width field: %10LG", expected);
  s21_sprintf(buffer_custom, "Double with width field: %10LG", expected);
...
START_TEST(test_s21_sprintf_G_combined_flags_and_width) {
  ...
  sprintf(buffer_standard, "Combined flags and width: %+10G", expected);
  s21_sprintf(buffer_custom, "Combined flags and width: %+10G", expected);
...
START_TEST(test_s21_sprintf_G_width_minus_sign) {
  ...
  sprintf(buffer_standard, "Width and minus sign: %-10G", expected);
  s21_sprintf(buffer_custom, "Width and minus sign: %-10G", expected);
...
START_TEST(test_s21_sprintf_G_width_precision_zero_padding) {
  char buffer_standard[100];
  char buffer_custom[100];
  long double expected = 434552423.3456789;

  sprintf(buffer_standard, "Width, precision, and zero padding: %010LG",
          expected);
  s21_sprintf(buffer_custom, "Width, precision, and zero padding: %010LG",
              expected);
...
START_TEST(test_s21_sprintf_LG_precision_minus_sign_negative_value) {
  char buffer_standard[100];
  char buffer_custom[100];
  long double expected = -42.3456789;

  sprintf(buffer_standard, "Precision and minus sign for negative value: %.3LG",
          expected);
  s21_sprintf(buffer_custom,
              "Precision and minus sign for negative value: %.3LG", expected);
...
START_TEST(test_s21_sprintf_G_width_plus_sign_negative_value) {
  ...
  sprintf(buffer_standard, "Width and plus sign for negative value: %+10G",
          expected);
  s21_sprintf(buffer_custom, "Width and plus sign for negative value: %+10G",
              expected);
...
Suite *sprintf_G_suite(void) {
  ...
  tcase_add_test(tc_core,
                 test_s21_sprintf_LG_precision_minus_sign_negative_value);
